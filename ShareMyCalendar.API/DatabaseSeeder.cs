...
public class DatabaseSeeder : IDatabaseSeeder
{
    ...
    public async Task Seed()
    {
        ...
        {
            _ = await _roleManager.CreateAsync(Roles.Admin);
        }

        if (await _roleManager.RoleExistsAsync(Roles.User.Name) == false)
        {
            _ = await _roleManager.CreateAsync(Roles.User);
        }

        if (await _userManager.FindByIdAsync(Admin.Id.ToString()) == null)
        {
            _ = await _userManager.CreateAsync(Admin, AdminPassword);
            _ = await _userManager.AddToRoleAsync(Admin, Roles.Admin.Name);
        }
    }
}
